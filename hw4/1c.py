...
def generate_episode(policy: np.ndarray, p_h=0.55, max_steps=1000000):
    state_history = []
    action_history = []
    reward_history = []
...
def monte_carlo(policy: np.ndarray, p_h, num_of_episodes=100, discount=1):
...
def main():
    p_h = 0.55
    values = monte_carlo(np.load(f'{p_h} policy.npy'), p_h=p_h)
